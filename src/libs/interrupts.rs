...
lazy_static! {
    static ref INTERRUPT_DESCRIPTOR_TABLE: InterruptDescriptorTable = {
       let mut interrupt_descriptor_table = InterruptDescriptorTable::new();
        ...
        unsafe {
            interrupt_descriptor_table.double_fault.set_handler_fn(double_fault_handler).set_stack_index(gdt::DOUBLE_FAULT_IST_INDEX);
        }
        interrupt_descriptor_table[InterruptIndex::Timer.as_usize()].set_handler_fn(timer_interrupt_handler);
        interrupt_descriptor_table[InterruptIndex::Keyboard.as_usize()].set_handler_fn(keyboard_interrupt_handler);
